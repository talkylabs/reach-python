...
test-docker:
	docker build -t talkylabs/reach-python .
	docker run talkylabs/reach-python pytest tests --ignore=tests/cluster
...
analysis:
	...
	. venv/bin/activate; flake8 --ignore=E402,F401,W391,W291,W293,W503,E203 talkylabs --max-line-length=300
...
test-with-coverage: prettier-check
	. venv/bin/activate; \
  	pytest --cov-config=setup.cfg --cov-report xml --cov=talkylabs tests --ignore=tests/cluster
...
docs:
	...
	-rm -rf docs/build
	. venv/bin/activate; sphinx-apidoc -f talkylabs -o docs/source/_rst
...
docker-build:
	docker build -t talkylabs/reach-python .
	docker tag talkylabs/reach-python talkylabs/reach-python:${GITHUB_TAG}
	docker tag talkylabs/reach-python talkylabs/reach-python:apidefs-${API_DEFINITIONS_SHA}
	docker tag talkylabs/reach-python talkylabs/reach-python:${CURRENT_TAG}

docker-push:
	docker push talkylabs/reach-python:${GITHUB_TAG}
	docker push talkylabs/reach-python:apidefs-${API_DEFINITIONS_SHA}
	docker push talkylabs/reach-python:${CURRENT_TAG}
