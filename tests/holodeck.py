...
from talkylabs.reach.http.request import Request
import platform
...
class Holodeck(HttpClient):
    ...
    def add_standard_headers(self, request):
        standard_headers = {
            "User-Agent": "reach-python/{} ({} {}) Python/{}".format(
                __version__,
                platform.system(),
                platform.machine(),
                platform.python_version(),
            ),
            "X-Reach-Client": "python-{}".format(__version__),
    ...
    def request(
        ...
        print(f"Len Holograms: {len(self._holograms)}")
        ...
        raise ReachRestException(404, url, message, method=method)
