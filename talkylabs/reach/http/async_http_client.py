...
from talkylabs.reach.http import AsyncHttpClient
from talkylabs.reach.http.request import Request as ReachRequest
from talkylabs.reach.http.response import Response

_logger = logging.getLogger("talkylabs.reach.http.async_http_client")


class AsyncReachHttpClient(AsyncHttpClient):
    """
    General purpose asynchronous HTTP Client for interacting with the Reach API
    """
    ...
    def __init__(
        ...
        """
        Constructor for the AsyncReachHttpClient
    ...
    async def request(
        ...
        if auth is not None:
            ...
            if headers is None:
                headers = {}
            headers[self.apiUserHeader] = auth[0]
            headers[self.apiKeyHeader]  = auth[1]
        ...
        if self.session:
            ...
            temp = True
        self._test_only_last_request = ReachRequest(**kwargs)
